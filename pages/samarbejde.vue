...
<ul class="flex flex-col gap-4 min-h-[258px]">
  <li v-for="(bulletpoint, index) in data.bulletpoint" :key="index" class="bulletpoint relative ml-16">{{bulletpoint}}</li>
</ul> 
    </article>
  </div>
...
<ul class="flex flex-col gap-4 min-h-[258px]">
  <li v-for="(bulletpoint, index) in data.bulletpoint2" :key="index" class="bulletpoint relative ml-16">{{bulletpoint}}</li>
</ul> 
<p class="sm:mt-0 sm:mb-0 mb-10">{{ data.text2[0].children[0].text }}
</p>
    </article>
